...
is_mixture = st.radio("Select composition type:", ["Single Component", "Mixture"]) == "Mixture"

st.subheader("GESAMP Scores")
if is_mixture:
    orgAc = st.text_input("Mixture Acute Toxicity (e.g. Score 4 - Moderately Toxic)", "")
    orgCh = st.text_input("Mixture Chronic Toxicity", "")
else:
    orgAc = st.text_input("Single Acute Toxicity", "")
    orgCh = st.text_input("Single Chronic Toxicity", "")

metalAc = st.selectbox("Metal Category (Acute)", ["None", "Category 1", "Category 2"])
metalCh = st.selectbox("Metal Category (Chronic)", ["None", "Category 1", "Category 2"])
...
st.markdown("---")
if st.button("Run Hazard Assessment"):
    st.success("Hazard logic and results will appear here.")
